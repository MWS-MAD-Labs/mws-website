...
export default function Affiliations({ title, text, logos, logosLabel }: AffiliationsProps) {
  return (
    <section
    ...
    >
      <span
        ...
        style={{ backgroundImage: affiliationShape, transform: 'translateX(35%)' }}
        aria-hidden="true"
      />
      <div className="relative z-10 mx-auto flex w-full max-w-[1240px] flex-col items-center">
        ...
        <div className="relative w-full overflow-hidden [-webkit-mask-image:linear-gradient(to_right,rgba(36,23,24,0)_0%,rgba(36,23,24,1)_10%,rgba(36,23,24,1)_90%,rgba(36,23,24,0)_100%)] [mask-image:linear-gradient(to_right,rgba(36,23,24,0)_0%,rgba(36,23,24,1)_10%,rgba(36,23,24,1)_90%,rgba(36,23,24,0)_100%)]">
          <div className="flex w-max animate-marqueeScroll items-center gap-20 motion-reduce:animate-none max-[980px]:gap-12 max-[680px]:gap-8">
            {[...logos, ...logos].map((src, index) => (
              <div key={`${src}-${index}`} className="shrink-0">
                <img
                  className="h-[75px] w-auto object-contain transition-transform duration-300 hover:scale-105 max-[980px]:h-[58px] max-[680px]:h-[46px]"
                  src={src}
